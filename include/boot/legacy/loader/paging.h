...
#define BIG_PAGE_SIZE 0x200000  // 2MiB
...
// For saving space, the loader creates the bigger 2MB pages!
// It identity maps the first 4GB (this way it ensures the VESA buffer is accessible)
static int IdentityPaging_Initialize() {
    uint64_t *pd = (uint64_t*)lmalloc_a(0x4000, 0x1000);
